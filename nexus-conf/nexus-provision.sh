#!/bin/bash
#
# By storing the date now, we can calculate the duration
# of provisioning at the end of this script.
start_seconds="$(date +%s)"

## package_check
# Build a bash array to pass all of the packages we want to install
# to a single apt-get command. This avoids doing all the leg work each
# time a package is set to install. It also allows us to easily comment
...
# Start with a bash array containing all packages we want to install
# in the virtual machine. We'll then loop through each of these
# and check individual status before adding them to the
# package_install_list array.
apt_package_check_list=(

    # PHP5
    # Our base packages for php5. As long as php5-fpm and php5-cli are
    # installed, there is no need to install the general php5 package,
    # which can sometimes install apache as a requirement.
    php5-fpm
    php5-cli
    php5-common # Common packages for php
    php5-dev # Dev packages for php
    php5-memcache # Extra PHP modules
    php5-imagick # Extra PHP modules
    php5-mcrypt # Extra PHP modules
    php5-mysql # Extra PHP modules
    php5-imap # Extra PHP modules
    php5-curl # Extra PHP modules
    php-pear # Extra PHP modules
    php5-gd # Extra PHP modules

    nginx # nginx is installed as the default web server

    memcached # memcached is made available for object caching

    mysql-server # mysql is the default database

    imagemagick # other packages
    subversion # other packages
    git-core # other packages
    zip # other packages
    unzip # other packages
    ngrep # other packages
    curl # other packages
    make # other packages
    vim # other packages
    colordiff # other packages
    postfix # other packages

    ntp # ntp service to keep clock current

    gettext # Req'd for i18n tools

    graphviz # Req'd for Webgrind

    # dos2unix
    # Allows conversion of DOS style line endings to something
    # we'll have less trouble with in Linux.
    dos2unix


    g++ # required for nodejs
    nodejs # nodejs for use by grunt
...
network_detection() {
    # Network Detection
    # Make an HTTP request to google.com to determine if outside access is
    # available to us. If 3 attempts with a timeout of 5 seconds are not
    # successful, then we'll skip a few things further in provisioning
    # rather than create a bunch of errors.
...
package_check() {
    # Loop through each of our packages that should be installed on the
    # system. If not yet installed, it should be added to the array of
    # packages to install.
    local pkg
...
package_install() {
    ...
    # Use debconf-set-selections to specify the default password for the
    # root MySQL account. This runs on every provision, even if MySQL
    # has been installed. If MySQL is already installed, it will not
    # affect anything.
    echo mysql-server mysql-server/root_password password "root" | debconf-set-selections
    echo mysql-server mysql-server/root_password_again password "root" | debconf-set-selections

    # Use debconf-set-selections to specify the selections in the postfix
    # setup. Set up as an 'Internet Site' with the host name 'vvv'.
    # Note that if your current Internet connection does not allow
    # communication over port 25, you will not be able to send mail,
    # even with postfix installed.
...
# Time for WordPress!
echo " "
echo "Installing/updating WordPress Stable, Trunk"

wordpress_default
wpsvn_check
wordpress_trunk

# Curious about WordPress Core development? Uncomment below!
# wordpress_develop
